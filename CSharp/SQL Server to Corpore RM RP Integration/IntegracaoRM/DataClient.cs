...

{
    ...
    public class DataClient
    {
        ...
        public async Task<string> ReadSchema(string DataServerName)
        {
            try
            {
                using (DataServer.IwsDataServerClient client = CreateClient(ServerAddress, UserName, Password))
                    return await client.GetSchemaAsync(DataServerName, Context);
            }
            catch (Exception ex)
            {
        ...
        public async Task<Tuple<DataSet, string>> ReadView(string DataServerName, string filter)
        {
            try
            {
                ...
                using (DataServer.IwsDataServerClient client = CreateClient(ServerAddress, UserName, Password))
                    viewData = await client.ReadViewAsync(DataServerName, filter, Context);
        ...
        public async Task<Tuple<DataSet, string>> ReadRecord(string DataServerName, string pkString)
        {
            try
            {
                ...
                // faz a chamada...
                using (DataServer.IwsDataServerClient client = CreateClient(ServerAddress, UserName, Password))
                    recordData = await client.ReadRecordAsync(DataServerName, pkString, Context);
          ...
          bool ignoreExtendedProps)
        {
            try
            {
                ...
                // salva...
                return await SaveRecord(DataServerName, xml);
            }
            catch (Exception ex)
            {
        ...
        public async Task<string[]> SaveRecord(string DataServerName, string dataSetXML)
        {
            ...
            try
            {
                // faz a chamada...
                using (DataServer.IwsDataServerClient client = CreateClient(ServerAddress, UserName, Password))
                    result = await client.SaveRecordAsync(DataServerName, dataSetXML, Context);
            }
            catch (Exception ex)
            {
                throw new Exception("Erro ao salvar o registro.\r\n\r\n" + ex.Message);
        ...
        private DataServer.IwsDataServerClient CreateClient(string serverAd0dress, string userName, string password)
        {
            // cria o cliente...
            string url = string.Format("{0}/wsDataServer/IwsDataServer", ServerAddress);
